...
| `ConversationCase` | the Interaction Case with which a Conversation gives way to a Sequence | optional Interaction Condition, Sequence identity, close or resume outcome | read from the top, first eligible case applies, and an unconditional case must come last; no default is required, since a Conversation with no eligible case stays open; evaluates committed Game State; generated wording has no authority | Conversation case definition/condition/Sequence diagnostics | [Dialogue](authoring/dialogue.md) |
...
| `CharacterDialogueDefinition` | optional Character dialogue profile | knowledge, Cover Stories, Relationships, Interaction Cases, authored alternatives, qualitative portrayal and state | omission preserves authored behaviour | dialogue capability diagnostics | [Dialogue](authoring/dialogue.md) |
...
| `SceneOpeningCase` | how a Scene reacts to its own Scene Opening | optional condition and Entrance, at most one outcome, optional operations | read from the top, first eligible case applies; an unconditional case must come last, and naming an Entrance counts as a condition; no default is required, since a Scene need not react; an Entrance filters to that door and so cannot apply at the start of a game; the start of a game is an opening, restoration never is | outcome/reference diagnostics | [Scene](recipes/world.ts) |
...
| `BranchCase` | one branch of a Sequence | optional condition and steps | the last case carries no condition and is the default, which a branch requires because it must reach somewhere | condition/ordering/cycle diagnostics | [Sequence](recipes/sequences.ts) |
