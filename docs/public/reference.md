...
The Continuation State carries it too, so a reload returns to the same
close-up. Restoring into a presented Detail View is not an arrival and starts
no arrival Sequence. A stored Detail View the Game Project no longer declares
is refused during validation with `save.state.detail-view`.
