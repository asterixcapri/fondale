...
- [x] Un Interaction Case sceglie esattamente una lista di operazioni
      dichiarative oppure un Game Behavior, mai entrambe.
- [x] Il Game Behavior è collocato sulla Game Definition pertinente e riceve un
      contesto temporaneo tipizzato di letture committed, bersaglio e operazioni
      controllate.
- [x] Il contesto non espone Game State grezzo, mutazione, DOM, PixiJS,
      renderer, input, clock o lifecycle della Game Session.
- [x] Promise, timer, casualità globale, rete e stato esterno mutabile non
      appartengono al contratto documentato del Game Behavior.
- [x] Le Game Operation richieste vengono validate e applicate come un unico
      gruppo atomico dopo il ritorno del comportamento.
- [x] Un comportamento che lancia o richiede un'operazione invalida non produce
      commit e porta la Game Session in `failed` con percorso e causa originale
      disponibili.
- [x] L'Example esercita almeno un Game Behavior sincrono attraverso
      l'interface pubblica e ne rende percepibile il risultato.
- [x] Il test adapter dimostra che lo stesso comportamento con lo stesso
      contesto produce gli stessi snapshot ed effetti.
- [x] La documentazione chiarisce scopo, limiti, operazioni disponibili,
